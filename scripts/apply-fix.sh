...
PHONESKY_DIR="/system/app/Phonesky"
PHONESKY_APK="$PHONESKY_DIR/Phonesky.apk"
TMP_APK="/data/local/tmp/_vending_stub.apk"

echo ""
echo "============================================"
echo "  InnovoP4 HomeLogic Fix — On-Device"
echo "============================================"
echo ""

# ── Check root ────────────────────────────────────────────────────────────────
if [ "$(id -u)" != "0" ]; then
    echo "[ERROR] Not running as root. Run 'adb root' first."
    exit 1
fi

# ── Check if already applied ─────────────────────────────────────────────────
if pm list packages 2>/dev/null | grep -q "com.android.vending"; then
    CURRENT_VER=$(dumpsys package com.android.vending 2>/dev/null | grep versionName | head -1 | tr -d ' ' | cut -d= -f2)
    case "$CURRENT_VER" in
        *stub*)
            echo "[OK] Stub already installed ($CURRENT_VER). Applying AOT only..."
            cmd package compile -m speed -f "$PACKAGE" 2>/dev/null && echo "[OK] AOT compilation applied." || echo "[WARN] AOT compilation failed."
            echo "[OK] Done."
            exit 0
            ;;
        *)
            echo "[WARN] Real Play Store ($CURRENT_VER) detected. Nothing to do."
            exit 0
            ;;
    esac
fi
...
# The base64-encoded vending-stub.apk (8.5KB)
...
echo "[INFO] Installing to $PHONESKY_DIR..."
mkdir -p "$PHONESKY_DIR"
cp "$TMP_APK" "$PHONESKY_APK"
chmod 755 "$PHONESKY_DIR"
chmod 644 "$PHONESKY_APK"
chown root:root "$PHONESKY_DIR" "$PHONESKY_APK"
rm -f "$TMP_APK"
echo "[OK] Stub APK installed."

# ── AOT compile ──────────────────────────────────────────────────────────────
echo "[INFO] Applying AOT compilation..."
cmd package compile -m speed -f "$PACKAGE" 2>/dev/null && echo "[OK] AOT compilation applied." || echo "[WARN] AOT compilation result unknown."

echo ""
echo "[OK] Fix applied. Reboot the panel to register com.android.vending:"
echo "     adb reboot"
echo ""
